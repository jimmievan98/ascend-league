...
{ name:“Atlanta”,    state:“GA”, status:“coming_soon” },
{ name:“Austin”,     state:“TX”, status:“coming_soon” },
{ name:“Dallas”,     state:“TX”, status:“coming_soon” },
{ name:“Tampa”,      state:“FL”, status:“coming_soon” },
{ name:“Orlando”,    state:“FL”, status:“coming_soon” },
{ name:“Phoenix”,    state:“AZ”, status:“coming_soon” },
{ name:“Miami”,      state:“FL”, status:“coming_soon” },
];
...
s.textContent = `*{box-sizing:border-box;margin:0;padding:0} html,body{height:100%;overflow-x:hidden;overscroll-behavior:none;-webkit-overflow-scrolling:touch} body{background:#f5f5f3;-webkit-tap-highlight-color:transparent;position:relative;overflow-y:auto} input[type="date"],input[type="time"]{ background:#fff; border:1.5px solid #d1d5db; border-radius:10px; padding:12px 14px; font-size:15px; color:#111; width:100%; outline:none; font-family:'DM Sans',sans-serif; -webkit-appearance:none; cursor:pointer; box-shadow:0 1px 3px rgba(0,0,0,.06); transition:border .15s,box-shadow .15s; } input[type="date"]:focus,input[type="time"]:focus{ border-color:#0369a1; box-shadow:0 0 0 3px rgba(3,105,161,.12); } input[type="time"]::-webkit-calendar-picker-indicator, input[type="date"]::-webkit-calendar-picker-indicator{ opacity:0.7; cursor:pointer; filter:invert(0.3); width:20px; height:20px; } .tscroll{overflow-x:auto;-webkit-overflow-scrolling:touch} @keyframes slideUp{from{transform:translateY(100%);opacity:0}to{transform:translateY(0);opacity:1}} @keyframes fadeIn{from{opacity:0}to{opacity:1}} @keyframes pulse{0%,100%{opacity:1}50%{opacity:.5}} @keyframes bounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-4px)}}`;
...
teamName:””, p2Name:””, p2Email:””, p2Phone:””,
division:””, agreed:false,
city:””, league_type:””
});

// City interest signup state
const [cityInterestEmail, setCityInterestEmail] = useState(””);
const [cityInterestSent,  setCityInterestSent]  = useState(””);
...
if(step===PHONE_STEP){ return; }
if(step===2){
if(!form.city){setErr(“Please select your city.”);return;}
}
if(step===3){
if(!form.league_type){setErr(“Please select a league type.”);return;}
}
if(step===4){
...
if(!form.division){setErr(“Please select your skill level.”);return;}
}
if(step===5){
...
try {
  ...
  if(form.city){
    try{
      const{data:cityRow}=await sb.from("cities").select("id").eq("name",form.city).single();
  ...
  const{data:team,error:teamErr}=await sb.from("teams").insert({
    ...
    city_id:cityId, league_type:form.league_type||"social"
  }).select().single();
...
const stepLabels = [””,“Create Account”,“Verify Phone”,“Select City”,“League Type”,“Your Info”,“Your Partner”,“Rules & Waiver”,“Review & Pay”];
const totalSteps = isOAuth ? 7 : 8;
const displayStep = isOAuth ? step : step;
...
```
      ...
      {step===2&&<>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px",marginBottom:"8px"}}>
          {CITIES.map(city=>{
            ...
            const isSelected = form.city===city.name;
            return(
              <div key={city.name} onClick={()=>isActive&&up("city",city.name)}
                style={{border:`2px solid ${isSelected?C.blue:isActive?"#e0e0e0":"#f0f0f0"}`,borderRadius:"12px",padding:"14px 12px",cursor:isActive?"pointer":"not-allowed",background:isSelected?C.blue+"10":isActive?C.white:"#fafafa",opacity:isActive?1:0.7,transition:"all .15s",position:"relative"}}>
                <div style={{fontSize:"14px",fontWeight:"700",color:isActive?C.text:"#aaa",marginBottom:"2px"}}>{city.name}</div>
                <div style={{fontSize:"11px",color:C.faint}}>{city.state}</div>
                {isActive
                  ? <div style={{position:"absolute",top:"8px",right:"8px",background:"#dcfce7",color:"#166534",fontSize:"9px",fontWeight:"700",padding:"2px 6px",borderRadius:"999px",letterSpacing:".5px"}}>ACTIVE</div>
                  : <div style={{position:"absolute",top:"8px",right:"8px",background:"#f3f4f6",color:"#9ca3af",fontSize:"9px",fontWeight:"700",padding:"2px 6px",borderRadius:"999px",letterSpacing:".5px"}}>SOON</div>
                }
              </div>
            );
          })}
          ...
          {cityInterestSent
            ...
            : <div style={{display:"flex",gap:"8px"}}>
                ...
                <button style={btn("#78350f","#fff",{fontSize:"12px",padding:"8px 14px",minHeight:"36px"})} onClick={async()=>{
                  if(!cityInterestEmail.trim())return;
                  try{await sb.from("city_interest").insert({email:cityInterestEmail});}catch(e){}
                  setCityInterestSent(true);
                }}>Notify me</button>
              </div>
          }
        </div>
      ...
      {step===3&&<>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
          {/* Social */}
          <div onClick={()=>up("league_type","social")}
            style={{border:`2px solid ${form.league_type==="social"?C.blue:C.border}`,borderRadius:"14px",padding:"16px",cursor:"pointer",background:form.league_type==="social"?C.blue+"10":C.white,transition:"all .15s"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"6px"}}>
              <div style={{fontSize:"16px",fontWeight:"800"}}>Social League</div>
              <div style={{background:"#dcfce7",color:"#166534",fontSize:"10px",fontWeight:"700",padding:"3px 8px",borderRadius:"999px"}}>OPEN</div>
            </div>
          ...
          <div style={{border:`2px solid #e0e0e0`,borderRadius:"14px",padding:"16px",cursor:"not-allowed",background:"#fafafa",opacity:0.7,position:"relative"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"6px"}}>
              ...
              <div style={{background:"#f3f4f6",color:"#9ca3af",fontSize:"10px",fontWeight:"700",padding:"3px 8px",borderRadius:"999px"}}>COMING SOON</div>
            </div>
            <div style={{fontSize:"12px",color:"#bbb",lineHeight:"1.6"}}>Rated competitive league using your official DUPR rating. Matches count toward your DUPR score. Launching soon.</div>
          </div>
        </div>
      </>}
          ...
          {["low","high"].map(d=>{
            ...
            return(
              <button key={d} onClick={()=>up("division",d)} style={{flex:1,padding:"16px 10px",borderRadius:"12px",border:`2px solid ${sel?dC(d):C.border}`,background:sel?dC(d):C.white,color:sel?"#fff":C.text,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",transition:"all .15s",textAlign:"center"}}>
                <div style={{fontSize:"17px",fontWeight:"800",marginBottom:"4px"}}>{label}</div>
                <div style={{fontSize:"11px",opacity:.75}}>{desc}</div>
              </button>
            );
          })}
        </div>
    ...
    <div style={{background:"#0d2137",borderRadius:"14px",padding:"16px",marginBottom:"16px",textAlign:"center"}}>
      ...
      <div style={{display:"flex",justifyContent:"center",gap:"0"}}>
        {[{v:cdTime.d,l:"Days"},{v:cdTime.h,l:"Hrs"},{v:cdTime.m,l:"Min"},{v:cdTime.s,l:"Sec"}].map(({v,l},i)=>(
          <div key={l} style={{flex:1,borderRight:i<3?"1px solid #1a3a55":"none",padding:"0 8px"}}>
            <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:"28px",fontWeight:"800",color:"#00BFFF",lineHeight:1}}>{String(v).padStart(2,"0")}</div>
            <div style={{fontSize:"9px",color:"#4a90b8",textTransform:"uppercase",letterSpacing:"1px",marginTop:"3px"}}>{l}</div>
          </div>
        ))}
      </div>
    </div>
...
const confirmMid = Object.keys(entry).find(k=>k.startsWith(”_*confirm*”))?.replace(”_*confirm*”,””) || null;
const closeConfirm = () => setEntry(e=>{const n={…e};Object.keys(n).filter(k=>k.startsWith(”_*confirm*”)).forEach(k=>delete n[k]);return n;});
...
<style>{`@keyframes ascend-pulse { 0%,100% { opacity:1; transform:scale(1); } 50%      { opacity:0.15; transform:scale(0.78); } } .ascend-loader { animation: ascend-pulse 2s ease-in-out infinite; }`}</style>
<img
